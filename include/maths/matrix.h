...
namespace srph {
template <class T, int M, int N> class matrix_t : protected std::array<T, M * N> {
  private:
  ...
  public:
    ...
    void operator+=(const matrix_t<T, M, N> &x) {
        std::transform(this->begin(), this->end(), x.begin(), this->begin(),
                       std::plus<T>());
    }
    ...
    void operator*=(const T &x) { scale(matrix_t<T, M, N>(x)); }
    ...
    // setters
    void set(int row, int column, const T &x) { (*this)[column * M + row] = x; }
...
decltype(S() * T()) dot(const matrix_t<S, M, N> &x, const matrix_t<T, M, N> &y) {
    return std::transform_reduce(x.begin(), x.end(), y.begin(),
                                 decltype(S() * T())(0));
}
