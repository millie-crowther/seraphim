...
namespace srph {
	template < class T, int M, int N >
	  ...
	  private:
		...
		template < int K, int P, typename ... Xs >
			...
			static_assert(K + P == M * N
				|| sizeof ... (Xs) > 0, "Not enough data in matrix constructor");
		...
		template < int K, typename ... Xs > void construct(const T & x, Xs ... xs) {
	  ...
	  matrix_t():matrix_t(T(0)) {
		...
		template < class X, class ... Xs > matrix_t(const X & x, Xs ... xs) {
			construct < 0 > (x, xs ...);
		}
		...
		template < class S > matrix_t < decltype(T() - S()
		), M, N > operator-(const matrix_t < S, M, N > &x) const {
			matrix_t < decltype(T() - S()
			), M, N > r;
			std::transform(this->begin(), this->end(), x.begin(),
				r.begin(), std::minus()
		...
		template < class S > matrix_t < decltype(T() * S()
		), M, N > scaled(const matrix_t < S, M, N > &x) const {
			matrix_t < decltype(T() * S()
			), M, N > r;
			std::transform(this->begin(), this->end(), x.begin(),
	...
	namespace vec {
		template < class S, class T, int M, int N > decltype(S() * T()
		) dot(const matrix_t < S, M, N > &x, const matrix_t < T, M, N > &y) {
			return std::transform_reduce(x.begin(), x.end(),
				y.begin(), decltype(S() * T())(0));
		}
		template < class T, int M, int N > T length(const matrix_t < T, M, N > &x) {
