...
namespace srph {
    struct command_buffer_t {
        ...
        template<class F>
        command_buffer_t(VkDevice device,
            ...
            if (vkAllocateCommandBuffers
                        (device, &alloc_info, &command_buffer) != VK_SUCCESS) {
        ...
        void submit(VkSemaphore wait_sema, VkSemaphore signal_sema,
    ...
    struct command_pool_t {
        ...
        std::shared_ptr<command_buffer_t> one_time_buffer(const F &f) const {
            return std::make_shared<command_buffer_t>(device,
                                                      command_pool, queue, VK_COMMAND_BUFFER_USAGE_ONE_TIME_SUBMIT_BIT,
                                                      f);
        }

        template<class F>
        std::shared_ptr<command_buffer_t>
        reusable_buffer(const F &f) const {
            return std::make_shared<command_buffer_t>(device,
                                                      command_pool,
                                                      queue, VK_COMMAND_BUFFER_USAGE_SIMULTANEOUS_USE_BIT, f);
        }
    };
}
