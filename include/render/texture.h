...
namespace srph {
 ...
 private:
		...
		device_t *device;
		 ...
		 std::unique_ptr < host_buffer_t < std::array < uint32_t,
		    8 >>> staging_buffer;
 ...
 public:
		// constructors and destructors
		 texture_t(uint32_t binding, device_t * device,
			   u32vec3_t size, VkImageUsageFlags usage,
			   VkFormatFeatureFlagBits format_feature,
			   VkDescriptorType descriptor_type);
		~texture_t();

		VkWriteDescriptorSet get_descriptor_write(VkDescriptorSet
							  desc_set) const;

		void record_write(VkCommandBuffer command_buffer);
		void write(u32vec3_t p, const std::array < uint32_t, 8 > &x);
		VkDescriptorSetLayoutBinding get_descriptor_layout_binding()
		    const;

		// static methods
		static void check_format_supported(VkPhysicalDevice
						   physical_device,
						   VkFormat format,
