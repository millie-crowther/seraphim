...
void srph_matter_to_global_position(const matter_t * m, vec3 * tx,
				    const vec3 * x);
...
void srph_matter_integrate_forces(matter_t * self, double t,
				  const vec3 * gravity);
...
void matter_apply_impulse(matter_t * a, matter_t * b, const vec3 * x,
			  const vec3 * j);
