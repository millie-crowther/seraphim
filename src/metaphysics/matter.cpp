		 ...
		 const vec3 * initial_position, bool is_uniform, bool is_static)
{
	...
	m->transform.position =
	    initial_position == NULL ? vec3_zero : *initial_position;
...
void srph_matter_calculate_sphere_bound(matter_t * self, double dt)
{
	...
	srph_matter_to_global_position(self, &self->bounding_sphere.c,
				       &midpoint);
	self->bounding_sphere.r =
	    vec3_length(&radius) + vec3_length(&self->v) * dt;
}
					 ...
					 srph_deform_type type)
{
	...
	if (type == srph_deform_type_collision) {
		...
		for (size_t i = 0; i < self->deformations.size; i++) {
			...
			if (vec3_distance(&x0, &deform->x0) <
			    SERAPHIM_DEFORM_MAX_SAMPLE_DENSITY) {
				return NULL;
			}
		}
	}
	...
	*deform = {
	...
	for (size_t i = 0; i < self->deformations.size; i++) {
		vec3_add(&deform->v, &deform->v,
			 &self->deformations.data[i]->v);
	}
...
void
srph_matter_to_global_position(const matter_t * m, vec3 * tx, const vec3 * x)
{
...
void
srph_matter_integrate_forces(matter_t * self, double t, const vec3 * gravity)
{
...
static void
offset_from_centre_of_mass(matter_t * self, vec3 * r, const vec3 * x)
{
	vec3 com;
	...
	if (!matter->is_inertia_tensor_valid) {
		if (matter->is_uniform && matter->sdf->is_inertia_tensor_valid) {
			...
			while (hits < SERAPHIM_SDF_VOLUME_SAMPLES) {
				vec3 x;
				x.x =
				    srph_random_f64_range(&rng, b->lower.x,
							  b->upper.x);
				x.y =
				    srph_random_f64_range(&rng, b->lower.y,
							  b->upper.y);
				x.z =
				    srph_random_f64_range(&rng, b->lower.z,
							  b->upper.z);

				if (!matter->is_uniform) {
					srph_matter_material(matter, &mat,
							     NULL);
				}
			...
			if (matter->is_uniform
			    && !matter->sdf->is_inertia_tensor_valid) {
				matter->sdf->inertia_tensor =
				    matter->inertia_tensor;
...
void
matter_apply_impulse(matter_t * a, matter_t * b, const vec3 * x, const vec3 * j)
{
	vec3 j_negative;
