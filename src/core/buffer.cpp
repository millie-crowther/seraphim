...
void buffer_create(buffer_t * self, uint32_t binding, device_t * device,
	...
	if (vkCreateBuffer
		(device->device, &buffer_info, nullptr, &self->buffer) != VK_SUCCESS) {
	...
	if (vkAllocateMemory
		(device->device, &alloc_info, nullptr, &self->memory) != VK_SUCCESS) {
		throw std::runtime_error("Error: Failed to allocate buffer memory.");
	}

	if (vkBindBufferMemory(device->device, self->buffer, self->memory,
			0) != VK_SUCCESS) {
