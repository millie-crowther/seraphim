...
mfloat_t *_mat2_new(mfloat_t * result, mfloat_t m11, mfloat_t m12,
...
mfloat_t *_mat3_new(mfloat_t * result, mfloat_t m11, mfloat_t m12,
...
mfloat_t _mat3_determinant(mfloat_t * m0) {
	...
	mfloat_t determinant = m11 * m22 * m33
		+ m12 * m23 * m31
		+ m13 * m21 * m32 - m11 * m23 * m32 - m12 * m21 * m33 - m13 * m22 * m31;
	return determinant;
}
...
mfloat_t *_mat4_new(mfloat_t * result, mfloat_t m11, mfloat_t m12,
...
mfloat_t *_mat4_inverse(mfloat_t * result, mfloat_t * m0) {
	...
	inverse[0] = m22 * m33 * m44
		- m22 * m43 * m34
		- m23 * m32 * m44 + m23 * m42 * m34 + m24 * m32 * m43 - m24 * m42 * m33;
	inverse[4] =
		-m12 * m33 * m44 + m12 * m43 * m34 + m13 * m32 * m44 -
		m13 * m42 * m34 - m14 * m32 * m43 + m14 * m42 * m33;
	inverse[8] =
		m12 * m23 * m44 - m12 * m43 * m24 - m13 * m22 * m44 +
		m13 * m42 * m24 + m14 * m22 * m43 - m14 * m42 * m23;
	inverse[12] =
		-m12 * m23 * m34 + m12 * m33 * m24 + m13 * m22 * m34 -
		m13 * m32 * m24 - m14 * m22 * m33 + m14 * m32 * m23;
	inverse[1] =
		-m21 * m33 * m44 + m21 * m43 * m34 + m23 * m31 * m44 -
		m23 * m41 * m34 - m24 * m31 * m43 + m24 * m41 * m33;
	inverse[5] =
		m11 * m33 * m44 - m11 * m43 * m34 - m13 * m31 * m44 +
		m13 * m41 * m34 + m14 * m31 * m43 - m14 * m41 * m33;
	inverse[9] =
		-m11 * m23 * m44 + m11 * m43 * m24 + m13 * m21 * m44 -
		m13 * m41 * m24 - m14 * m21 * m43 + m14 * m41 * m23;
	inverse[13] =
		m11 * m23 * m34 - m11 * m33 * m24 - m13 * m21 * m34 +
		m13 * m31 * m24 + m14 * m21 * m33 - m14 * m31 * m23;
	inverse[2] =
		m21 * m32 * m44 - m21 * m42 * m34 - m22 * m31 * m44 +
		m22 * m41 * m34 + m24 * m31 * m42 - m24 * m41 * m32;
	inverse[6] =
		-m11 * m32 * m44 + m11 * m42 * m34 + m12 * m31 * m44 -
		m12 * m41 * m34 - m14 * m31 * m42 + m14 * m41 * m32;
	inverse[10] =
		m11 * m22 * m44 - m11 * m42 * m24 - m12 * m21 * m44 +
		m12 * m41 * m24 + m14 * m21 * m42 - m14 * m41 * m22;
	inverse[14] =
		-m11 * m22 * m34 + m11 * m32 * m24 + m12 * m21 * m34 -
		m12 * m31 * m24 - m14 * m21 * m32 + m14 * m31 * m22;
	inverse[3] =
		-m21 * m32 * m43 + m21 * m42 * m33 + m22 * m31 * m43 -
		m22 * m41 * m33 - m23 * m31 * m42 + m23 * m41 * m32;
	inverse[7] =
		m11 * m32 * m43 - m11 * m42 * m33 - m12 * m31 * m43 +
		m12 * m41 * m33 + m13 * m31 * m42 - m13 * m41 * m32;
	inverse[11] =
		-m11 * m22 * m43 + m11 * m42 * m23 + m12 * m21 * m43 -
		m12 * m41 * m23 - m13 * m21 * m42 + m13 * m41 * m22;
	inverse[15] =
		m11 * m22 * m33 - m11 * m32 * m23 - m12 * m21 * m33 +
...
mfloat_t *_mat4_perspective(mfloat_t * result, mfloat_t fov_y,
...
vec2 *vec2_bezier4(vec2 * result, vec2 * v0, vec2 * v1, vec2 * v2, vec2 * v3,
	...
	return (vec2 *) _vec2_bezier4((mfloat_t *) result, (mfloat_t *) v0,
		(mfloat_t *) v1, (mfloat_t *) v2, (mfloat_t *) v3, f);
}
...
vec3 *vec3_subtract(vec3 * result, const vec3 * v0, const vec3 * v1) {
	return (vec3 *) _vec3_subtract((mfloat_t *) result,
		(const mfloat_t *) v0, (const mfloat_t *) v1);
}
...
vec3 *vec3_multiply_mat3(vec3 * result, const vec3 * v0, const mat3 * m0) {
	return (vec3 *) _vec3_multiply_mat3((mfloat_t *) result,
		(const mfloat_t *) v0, (const mfloat_t *) m0);
}
...
mat4 *mat4_translation(mat4 * result, const mat4 * m0, const vec3 * v0) {
	return (mat4 *) _mat4_translation((mfloat_t *) result,
		(const mfloat_t *) m0, (const mfloat_t *) v0);
}
...
mat4 *mat4_look_at(mat4 * result, vec3 * position, vec3 * target, vec3 * up) {
	return (mat4 *) _mat4_look_at((mfloat_t *) result,
		(mfloat_t *) position, (mfloat_t *) target, (mfloat_t *) up);
}
...
mfloat_t exponential_ease_in_out(mfloat_t f) {
	...
	if (f < MFLOAT_C(0.5)) {
		...
		a = -MFLOAT_C(0.5) * MPOW(MFLOAT_C(2.0),
			-MFLOAT_C(20.0) * f + MFLOAT_C(10.0)) + MFLOAT_C(1.0);
	}
	return a;
}
...
mfloat_t back_ease_in_out(mfloat_t f) {
	...
	if (f < MFLOAT_C(0.5)) {
		...
		a = MFLOAT_C(0.5) * (MFLOAT_C(1.0) -
			(a * a * a - a * MSIN(f * MPI))) + MFLOAT_C(0.5);
	}
	return a;
}
