...
void srph_transform_to_global_position(const transform_t * tf, vec3 * tx, const vec3 * x){
...
void srph_transform_to_global_direction(const transform_t * tf, vec3 * tx, const vec3 * x){
    vec3_multiply_quat(tx, x, &tf->rotation);
}
