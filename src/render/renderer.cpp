		       ...
		       uint32_t max_image_size)
{
	...
	fragment_shader_code =
	    file_load_text("../src/render/shader/frag.glsl", NULL);
	vertex_shader_code =
	    file_load_text("../src/render/shader/vert.glsl", NULL);

	vkGetDeviceQueue(device->device, device->present_family, 0,
			 &present_queue);
...
renderer_t::~renderer_t()
{
	...
	vkDestroyDescriptorSetLayout(device->device, descriptor_layout,
				     nullptr);

	cleanup_swapchain();

	vkDestroyPipeline(device->device, compute_pipeline, nullptr);
	vkDestroyPipelineLayout(device->device, compute_pipeline_layout,
				nullptr);

	for (int i = 0; i < frames_in_flight; i++) {
		vkDestroySemaphore(device->device, image_available_semas[i],
				   nullptr);
		vkDestroySemaphore(device->device, compute_done_semas[i],
				   nullptr);
		vkDestroySemaphore(device->device, render_finished_semas[i],
				   nullptr);
...
void renderer_t::create_compute_pipeline()
{
	...
	if (vkCreatePipelineLayout
	    (device->device, &pipeline_layout_info, nullptr,
	...
	if (vkCreateComputePipelines
	    (device->device, VK_NULL_HANDLE, 1, &pipeline_create_info, nullptr,
...
void renderer_t::create_render_pass()
{
	...
	if (vkCreateRenderPass
	    (device->device, &render_pass_info, nullptr,
	     &render_pass) != VK_SUCCESS) {
		throw
		    std::runtime_error("Error: Failed to create render pass.");
	}
}
...
void renderer_t::create_graphics_pipeline()
{
	...
	const VkPipelineColorBlendStateCreateInfo colour_blend_const =
	    colour_blend_info;
...
void renderer_t::create_framebuffers()
{
	...
	for (uint32_t i = 0; i < swapchain->get_size(); i++) {
		...
		if (vkCreateFramebuffer
		    (device->device, &framebuffer_info, nullptr,
...
void renderer_t::create_descriptor_pool()
{
	...
	if (vkCreateDescriptorPool
	    (device->device, &pool_info, nullptr, &desc_pool) != VK_SUCCESS) {
		throw
		    std::runtime_error
...
void renderer_t::create_descriptor_set_layout()
{
	...
	if (vkCreateDescriptorSetLayout
	    (device->device, &layout_info, nullptr,
...
void renderer_t::create_sync()
{
	...
	for (int i = 0; i < frames_in_flight; i++) {
		result |=
		    vkCreateSemaphore(device->device, &create_info, nullptr,
...
void renderer_t::render()
{
	...
	vkAcquireNextImageKHR(device->device, swapchain->get_handle(),
			      ...
			      image_available_semas[current_frame],
...
VkShaderModule renderer_t::create_shader_module(std::string code)
{
	...
	if (vkCreateShaderModule
	    (device->device, &create_info, nullptr,
...
void renderer_t::handle_requests(uint32_t frame)
{
 ...
 for (auto & call:calls) {
		if (call.is_valid()) {
			...
			auto response =
			    get_response(call, &substances[substance_index]);
			patch_buffer->write_element(response.get_patch(),
						    call.get_index());

			u32vec3_t p =
			    u32vec3_t(call.get_index() % patch_image_size,
				      (call.get_index() %
				       (patch_image_size * patch_image_size)) /
				      patch_image_size,
				      call.get_index() / patch_image_size /
				      patch_image_size) * patch_sample_size;
...
void renderer_t::create_buffers()
{
	...
	patch_buffer =
	    std::make_unique < device_buffer_t < response_t::patch_t >> (1,
									 ...
									 number_of_patches);
	call_buffer =
	    std::make_unique < device_buffer_t < call_t >> (2, device,
							    number_of_calls);
	light_buffer =
	    std::make_unique < device_buffer_t < light_t >> (3, device, s);
	substance_buffer =
	    std::make_unique < device_buffer_t < substance_t::data_t >> (4,
									 device,
									 s);
	pointer_buffer =
...
response_t renderer_t::get_response(const call_t & call,
				    substance_t * substance)
{
	...
	if (response_cache.count(call) == 0) {
		auto result =
		    response_cache.emplace(call, response_t(call, substance));
		if (std::get < 1 > (result)) {
