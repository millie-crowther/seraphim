...
static void window_resize_callback(GLFWwindow * glfw_window, int width,
				   int height)
{
...
window_t::window_t(u32vec2_t size)
{
	...
	window =
	    glfwCreateWindow(size[0], size[1], "Seraphim", nullptr, nullptr);

	if (window == nullptr) {
		throw
		    std::runtime_error("Error: Failed to create main window.");
	}
