...
void renderer_t::create_compute_pipeline() {
    ...
    if (!shader_create(&compute_shader, "../frontend/comp.glsl", device, VK_SHADER_STAGE_COMPUTE_BIT)){
...
void renderer_t::create_graphics_pipeline() {
    if (!shader_create(&vertex_shader, "../frontend/vert.glsl", device, VK_SHADER_STAGE_VERTEX_BIT)){
        PANIC("Error: Failed to create vertex shader");
    }

    if (!shader_create(&fragment_shader, "../frontend/frag.glsl", device, VK_SHADER_STAGE_FRAGMENT_BIT)){
...
void renderer_t::render() {
    ...
    // write substances
    std::vector<substance_data_t> substance_data;
    ...
    std::sort(substance_data.begin(), substance_data.end(), substance_data_t::comparator_t());
